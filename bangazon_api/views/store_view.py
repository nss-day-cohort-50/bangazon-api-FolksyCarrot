        ...
        if request.method == 'POST':
            try: 
                ...
                try:
                    ...
                    return Response({'message':'it worked'}, status.HTTP_226_IM_USED)
        elif request.method == 'DELETE':
            try:
                favorite = Favorite.objects.get(customer = customer, store = store)
                favorite.delete()
                return Response({'message':'it worked'}, status = status.HTTP_204_NO_CONTENT)
            except:
                return Response({'message':'it did not work'}, status.HTTP_403_FORBIDDEN)
